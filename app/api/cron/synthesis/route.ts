 ...
 * asks Groq to extract explicit rules from patterns, saves to
...
Return ONLY the RULE lines, one per line. No explanation, no preamble.`
  ...
  const groqResult = await groqTextComplete(prompt, 600)
  if (!groqResult) {
    console.error('[synthesis] all Groq models failed')
    return NextResponse.json({ status: 'error', error: 'all Groq models failed' }, { status: 500 })
  }
  const rulesText = groqResult.text
  console.log(`[synthesis] rules generated by ${groqResult.model}`)
